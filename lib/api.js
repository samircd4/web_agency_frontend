/**
 * API client to communicate with the Django REST Framework backend.
 */
export const getApiBaseUrl = () => {
    if (typeof window !== "undefined") {
        const host = window.location.hostname;
        if (host === "localhost" || host === "127.0.0.1") {
            return "http://localhost:8000";
        }
        return "https://api.drpythonsolutions.com";
    }
    if (process.env.NEXT_PUBLIC_API_BASE_URL) return process.env.NEXT_PUBLIC_API_BASE_URL;
    return process.env.NODE_ENV === "production" ? "https://api.drpythonsolutions.com" : "http://localhost:8000";
};
export const API_BASE_URL = getApiBaseUrl().replace(/\/$/, "");

// ─── Token helpers ─────────────────────────────────────────────────────────

export function getCookie(name) {
    if (typeof window === "undefined") return null;
    const value = `; ${document.cookie}`;
    const parts = value.split(`; ${name}=`);
    if (parts.length === 2) return parts.pop().split(";").shift();
    return null;
}

export function getAuthHeaders() {
    if (typeof window === "undefined") return {};

    const token =
        localStorage.getItem("access_token") || getCookie("admin_session");

    const looksLikeJwt = typeof token === "string" && token.split(".").length === 3;
    if (token && !token.startsWith("mock-") && looksLikeJwt) {
        return { Authorization: `Bearer ${token}` };
    }
    return {};
}

export function isAuthenticated() {
    if (typeof window === "undefined") return false;
    const token =
        localStorage.getItem("access_token") || getCookie("admin_session");
    return !!token && !token.startsWith("mock-");
}

function clearTokens() {
    localStorage.removeItem("access_token");
    localStorage.removeItem("refresh_token");
    localStorage.removeItem("chat_thread_id");
    document.cookie =
        "admin_session=; path=/; expires=Thu, 01 Jan 1970 00:00:01 GMT;";
}

// ─── Refresh lock ──────────────────────────────────────────────────────────

let _refreshPromise = null;

async function refreshAccessToken() {
    if (_refreshPromise) return _refreshPromise;

    _refreshPromise = _doRefresh().finally(() => {
        _refreshPromise = null;
    });

    return _refreshPromise;
}

async function _doRefresh() {
    try {
        const refresh = localStorage.getItem("refresh_token");
        if (!refresh || refresh.startsWith("mock-")) return null;

        const res = await fetch(`${API_BASE_URL}/auth/token/refresh/`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ refresh }),
        });

        if (!res.ok) {
            clearTokens();
            if (typeof window !== "undefined") window.location.href = "/login";
            return null;
        }

        const data = await res.json();
        if (!data?.access) return null;

        localStorage.setItem("access_token", data.access);
        document.cookie = `admin_session=${data.access}; path=/; max-age=86400; SameSite=Lax; Secure`;
        return data.access;
    } catch {
        clearTokens();
        return null;
    }
}

// ─── Core request wrapper ──────────────────────────────────────────────────

async function request(url, options = {}) {
    const { _retried, skipAuth, ...restOptions } = options;

    const isFormDataBody =
        typeof FormData !== "undefined" && options.body instanceof FormData;

    const headers = {
        ...(isFormDataBody ? {} : { "Content-Type": "application/json" }),
        ...(skipAuth ? {} : getAuthHeaders()),
        ...options.headers,
    };

    const config = {
        ...restOptions,
        headers,
    };

    const finalUrl = url.startsWith("/") ? `${API_BASE_URL}${url}` : url;

    let response;
    try {
        response = await fetch(finalUrl, config);
    } catch (networkError) {
        const message =
            networkError instanceof TypeError
                ? `Cannot reach server — is the backend running? (${finalUrl})`
                : networkError.message || "Network error";

        if (typeof window !== "undefined") {
            console.error("Network error", {
                url: finalUrl,
                error: networkError.message,
            });
        }

        const error = new Error(message);
        error.status = 0;
        error.data = null;
        throw error;
    }

    if (!response.ok) {
        let errorData = null;
        try {
            errorData = await response.json();
        } catch {
            errorData = { detail: response.statusText };
        }

        // ── 401 handling ────────────────────────────────────────────────────────
        if (response.status === 401 && typeof window !== "undefined") {
            const detail = String(errorData?.detail || "").toLowerCase();
            const code = errorData?.code || "";

            const isExpiredToken =
                code === "token_not_valid" || detail.includes("token not valid");

            const isNoCredentials =
                detail.includes("authentication credentials were not provided") ||
                detail.includes("not provided");

            if (isExpiredToken && !_retried) {
                const refreshed = await refreshAccessToken();
                if (refreshed) {
                    return request(url, { ...restOptions, _retried: true });
                }
            } else if (isNoCredentials) {
                clearTokens();
                const path = window.location.pathname;
                if (path.startsWith("/admin") && path !== "/admin/login" && path !== "/admin/register") {
                    window.location.href = "/admin/login";
                }
                return;
            }
        }

        const message =
            errorData?.detail ||
            errorData?.message ||
            response.statusText ||
            `API request failed (HTTP ${response.status})`;

        const error = new Error(message);
        error.status = response.status;
        error.data = errorData;

        if (typeof window !== "undefined") {
            // 401 = wrong credentials / unauthenticated (expected, caller shows UI message)
            // 404 = resource doesn't exist (expected, caller handles it)
            // 405 = method not allowed (expected during partial backend rollouts)
            // Don't flood the console with these — only log unexpected server errors
            const silentStatuses = [401, 404, 405];
            if (!silentStatuses.includes(response.status)) {
                console.error("API error:", message, {
                    url: finalUrl,
                    status: response.status,
                    data: errorData,
                });
            }
        }


        throw error;
    }

    if (response.status === 204) {
        return null;
    }

    return response.json();
}

// ─── Helper Functions ─────────────────────────────────────────────────────

export function getFullMediaUrl(url) {
    if (!url) return '';
    const s = String(url).trim();
    if (s.includes("/media/")) {
        const idx = s.indexOf("/media/");
        const rel = s.substring(idx);
        const base = getApiBaseUrl().replace(/\/$/, "");
        return `${base}${rel}`;
    }
    return s;
}

export function getFullAvatarUrl(avatarUrl) {
    if (!avatarUrl) return null;
    const s = String(avatarUrl).trim();
    if (s.includes("/media/")) {
        const idx = s.indexOf("/media/");
        const rel = s.substring(idx);
        return `${API_BASE_URL}${rel}`;
    }
    if (s.startsWith("http://") || s.startsWith("https://")) {
        return s;
    }
    return `${API_BASE_URL}${s.startsWith("/") ? "" : "/"}${s}`;
}

/**
 * Build a WebSocket URL for a given chat thread ID.
 * Derives the host from API_BASE_URL so it works correctly in all
 * environments (localhost, staging, production) without any hardcoded ports.
 *
 * Example:
 *   API_BASE_URL = "http://localhost:8000"  → "ws://localhost:8000/ws/chat/3/"
 *   API_BASE_URL = "https://api.drpythonsolutions.com" → "wss://api.drpythonsolutions.com/ws/chat/3/"
 */
export function getWebSocketUrl(threadId) {
    // Replace http(s):// with ws(s):// — preserves host, port, and path prefix.
    const wsBase = API_BASE_URL
        .replace(/^https:\/\//i, 'wss://')
        .replace(/^http:\/\//i, 'ws://');
    return `${wsBase}/ws/chat/${threadId}/`;
}

/**
 * Build a WebSocket URL for the public guest chat endpoint.
 * Uses a UUID session_id from localStorage to authenticate the connection.
 * Example: "ws://localhost:8000/ws/guest/?session_id=<uuid>"
 */
export function getGuestWebSocketUrl(sessionId) {
    const wsBase = API_BASE_URL
        .replace(/^https:\/\//i, 'wss://')
        .replace(/^http:\/\//i, 'ws://');
    return `${wsBase}/ws/guest/?session_id=${sessionId}`;
}

/**
 * Build a WebSocket URL for the admin guest watch endpoint.
 * Example: "ws://localhost:8000/ws/guest-watch/"
 */
export function getGuestWatchWebSocketUrl() {
    const wsBase = API_BASE_URL
        .replace(/^https:\/\//i, 'wss://')
        .replace(/^http:\/\//i, 'ws://');
    return `${wsBase}/ws/guest-watch/`;
}


// ─── Public API surface ────────────────────────────────────────────────────

export const api = {
    // ─── Auth ────────────────────────────────────────────────────────────────

    async login(username, password, otp) {
        const body = { username, password };
        if (otp) body.otp = otp;

        const res = await request("/auth/token/", {
            method: "POST",
            body: JSON.stringify(body),
            skipAuth: true,
        });
        if (res.access) {
            localStorage.setItem("access_token", res.access);
            localStorage.setItem("refresh_token", res.refresh);
            document.cookie = `admin_session=${res.access}; path=/; max-age=86400; SameSite=Lax; Secure`;
        }
        return res;
    },

    async googleLogin(credential) {
        const res = await request("/auth/google/", {
            method: "POST",
            body: JSON.stringify({ credential }),
            skipAuth: true,
        });
        if (res.access) {
            localStorage.setItem("access_token", res.access);
            localStorage.setItem("refresh_token", res.refresh);
            document.cookie = `admin_session=${res.access}; path=/; max-age=86400; SameSite=Lax; Secure`;
        }
        return res;
    },

    async register(registrationData) {
        const res = await request("/auth/register/", {
            method: "POST",
            body: JSON.stringify(registrationData),
            skipAuth: true,
        });
        if (res.access) {
            localStorage.setItem("access_token", res.access);
            localStorage.setItem("refresh_token", res.refresh);
            document.cookie = `admin_session=${res.access}; path=/; max-age=86400; SameSite=Lax; Secure`;
        }
        return res;
    },

    async logout() {
        const refreshToken = localStorage.getItem('refresh_token');
        try {
            if (refreshToken && !refreshToken.startsWith("mock-")) {
                await request("/auth/logout/", {
                    method: "POST",
                    body: JSON.stringify({ refresh: refreshToken }),
                });
            }
        } catch (error) {
            console.error('Logout API call failed:', error);
        } finally {
            clearTokens();
        }
    },

    async getMe() {
        return request("/auth/me/");
    },

    async switchRole(activeRole) {
        return request("/auth/switch-role/", {
            method: "POST",
            body: JSON.stringify({ active_role: activeRole }),
        });
    },

    async becomeSeller(data = {}) {
        return request("/auth/become-seller/", {
            method: "POST",
            body: JSON.stringify(data),
        });
    },

    async becomeBuyer(data = {}) {
        return request("/auth/become-buyer/", {
            method: "POST",
            body: JSON.stringify(data),
        });
    },

    // ─── Seller API Methods ──────────────────────────────────────────
    async getSellerStats() {
        return request("/seller/stats/");
    },
    async getSellerProjects() {
        return request("/seller/projects/");
    },
    async getSellerProjectDetail(id) {
        return request(`/seller/projects/${id}/`);
    },
    async updateSellerProject(id, data) {
        const isFormData = data instanceof FormData;
        return request(`/seller/projects/${id}/`, {
            method: "PATCH",
            body: isFormData ? data : JSON.stringify(data),
        });
    },
    async addSellerMilestone(projectId, data) {
        return request(`/seller/projects/${projectId}/milestones/`, {
            method: "POST",
            body: JSON.stringify(data),
        });
    },
    async updateSellerMilestone(projectId, milestoneId, data) {
        return request(`/seller/projects/${projectId}/milestones/${milestoneId}/`, {
            method: "PATCH",
            body: JSON.stringify(data),
        });
    },
    async deleteSellerMilestone(projectId, milestoneId) {
        return request(`/seller/projects/${projectId}/milestones/${milestoneId}/`, {
            method: "DELETE",
        });
    },
    async uploadSellerProjectFile(projectId, formData) {
        return request(`/seller/projects/${projectId}/files/`, {
            method: "POST",
            body: formData,
        });
    },
    async deleteSellerProjectFile(projectId, fileId) {
        return request(`/seller/projects/${projectId}/files/${fileId}/`, {
            method: "DELETE",
        });
    },
    async sendSellerProposal(projectId, data) {
        return request(`/seller/projects/${projectId}/proposals/`, {
            method: "POST",
            body: JSON.stringify(data),
        });
    },
    async updateSellerProposal(projectId, proposalId, data) {
        return request(`/seller/projects/${projectId}/proposals/${proposalId}/`, {
            method: "PATCH",
            body: JSON.stringify(data),
        });
    },
    async sendExistingSellerProposal(projectId, proposalId) {
        return request(`/seller/projects/${projectId}/proposals/${proposalId}/send/`, {
            method: "POST",
        });
    },
    async sendSellerInvoice(projectId, data) {
        return request(`/seller/projects/${projectId}/invoices/`, {
            method: "POST",
            body: JSON.stringify(data),
        });
    },
    async updateSellerInvoice(projectId, invoiceId, data) {
        return request(`/seller/projects/${projectId}/invoices/${invoiceId}/`, {
            method: "PATCH",
            body: JSON.stringify(data),
        });
    },
    async sendExistingSellerInvoice(projectId, invoiceId) {
        return request(`/seller/projects/${projectId}/invoices/${invoiceId}/send/`, {
            method: "POST",
        });
    },
    async createSellerInvoiceItem(projectId, invoiceId, data) {
        return request(`/seller/projects/${projectId}/invoices/${invoiceId}/items/`, {
            method: "POST",
            body: JSON.stringify(data),
        });
    },
    async updateSellerInvoiceItem(projectId, invoiceId, itemId, data) {
        return request(`/seller/projects/${projectId}/invoices/${invoiceId}/items/${itemId}/`, {
            method: "PATCH",
            body: JSON.stringify(data),
        });
    },
    async deleteSellerInvoiceItem(projectId, invoiceId, itemId) {
        return request(`/seller/projects/${projectId}/invoices/${invoiceId}/items/${itemId}/`, {
            method: "DELETE",
        });
    },
    async getSellerServices() {
        return request("/seller/services/");
    },
    async createSellerService(data) {
        const isFormData = data instanceof FormData;
        return request("/seller/services/", {
            method: "POST",
            body: isFormData ? data : JSON.stringify(data),
        });
    },
    async updateSellerService(id, data) {
        const isFormData = data instanceof FormData;
        return request(`/seller/services/${id}/`, {
            method: "PATCH",
            body: isFormData ? data : JSON.stringify(data),
        });
    },
    async deleteSellerService(id) {
        return request(`/seller/services/${id}/`, {
            method: "DELETE",
        });
    },
    async getSellerServiceDetail(id) {
        return request(`/seller/services/${id}/`);
    },

    async getSellerPortfolio() {
        return request("/seller/portfolio/");
    },
    async createSellerPortfolio(data) {
        const isFormData = data instanceof FormData;
        return request("/seller/portfolio/", {
            method: "POST",
            body: isFormData ? data : JSON.stringify(data),
        });
    },
    async updateSellerPortfolio(id, data) {
        const isFormData = data instanceof FormData;
        return request(`/seller/portfolio/${id}/`, {
            method: "PATCH",
            body: isFormData ? data : JSON.stringify(data),
        });
    },
    async deleteSellerPortfolio(id) {
        return request(`/seller/portfolio/${id}/`, {
            method: "DELETE",
        });
    },
    async getSellerBlog() {
        return request("/seller/blog/");
    },
    async createSellerBlog(data) {
        const isFormData = data instanceof FormData;
        return request("/seller/blog/", {
            method: "POST",
            body: isFormData ? data : JSON.stringify(data),
        });
    },
    async updateSellerBlog(id, data) {
        const isFormData = data instanceof FormData;
        return request(`/seller/blog/${id}/`, {
            method: "PATCH",
            body: isFormData ? data : JSON.stringify(data),
        });
    },
    async deleteSellerBlog(id) {
        return request(`/seller/blog/${id}/`, {
            method: "DELETE",
        });
    },


    async updateMe(data) {
        const formData = new FormData();
        for (const key in data) {
            if (data[key] !== undefined && data[key] !== null) {
                formData.append(key, data[key]);
            }
        }
        return request("/auth/me/", { method: "PATCH", body: formData });
    },


    async checkUsername(username) {
        return request(`/auth/check-username/?username=${encodeURIComponent(username)}`, { skipAuth: true });
    },

    async changePassword(data) {
        return request("/auth/change-password/", {
            method: "POST",
            body: JSON.stringify(data),
        });
    },

    async otpGenerate() {
        return request("/auth/otp/generate/", { method: "POST" });
    },

    async otpVerify(data) {
        return request("/auth/otp/verify/", {
            method: "POST",
            body: JSON.stringify(data),
        });
    },

    async otpDisable(data) {
        return request("/auth/otp/disable/", {
            method: "POST",
            body: JSON.stringify(data),
        });
    },

    async getActiveSessions() {
        const refreshToken = localStorage.getItem("refresh_token");
        return request("/auth/sessions/", {
            method: "POST",
            body: JSON.stringify({ refresh_token: refreshToken }),
        });
    },

    async revokeSession(id) {
        const refreshToken = localStorage.getItem("refresh_token");
        return request(`/auth/sessions/${id}/revoke/`, {
            method: "POST",
            body: JSON.stringify({ refresh_token: refreshToken }),
        });
    },

    async revokeAllSessions() {
        const refreshToken = localStorage.getItem("refresh_token");
        return request("/auth/sessions/revoke-all/", {
            method: "POST",
            body: JSON.stringify({ refresh_token: refreshToken }),
        });
    },

    // ─── Public CMS ──────────────────────────────────────────────────────────

    async getBlogPosts(params = {}) {
        let url = "/public/blog/";
        const query = new URLSearchParams();
        if (typeof params === 'string' && params) {
            query.append('tags__name', params);
        } else if (typeof params === 'object' && params !== null) {
            if (params.tag && params.tag !== 'All') query.append('tags__name', params.tag);
            if (params.search) query.append('search', params.search);
        }
        const str = query.toString();
        if (str) url += `?${str}`;
        return request(url, { skipAuth: true });
    },

    async getBlogPostDetail(identifier) {
        return request(`/public/blog/${encodeURIComponent(identifier)}/`, { skipAuth: true });
    },

    async likeBlogPost(identifier, action = "like") {
        return request(`/public/blog/${encodeURIComponent(identifier)}/like/`, {
            method: "POST",
            body: JSON.stringify({ action }),
            skipAuth: true,
        });
    },

    async getBlogComments(identifier) {
        return request(`/public/blog/${encodeURIComponent(identifier)}/comments/`, { skipAuth: true });
    },

    async createBlogComment(identifier, commentData) {
        return request(`/public/blog/${encodeURIComponent(identifier)}/comments/`, {
            method: "POST",
            body: JSON.stringify(commentData),
            skipAuth: true,
        });
    },

    async getCategories(type = '') {
        const url = type ? `/public/categories/?type=${encodeURIComponent(type)}` : '/public/categories/';
        return request(url, { skipAuth: true });
    },

    async getPortfolioItems(params = {}) {
        let url = "/public/portfolio/";
        const query = new URLSearchParams();
        if (typeof params === 'string' && params) {
            if (params !== 'All') query.append('category', params);
        } else if (typeof params === 'object' && params !== null) {
            if (params.category && params.category !== 'All') query.append('category', params.category);
            if (params.tag) query.append('tags__name', params.tag);
            if (params.search) query.append('search', params.search);
        }
        const str = query.toString();
        if (str) url += `?${str}`;
        return request(url, { skipAuth: true });
    },

    async getPortfolioItemDetail(identifier) {
        return request(`/public/portfolio/${encodeURIComponent(identifier)}/`, { skipAuth: true });
    },

    // ─── Public Lead Submission ───────────────────────────────────────────────

    async createLead(leadData) {
        return request("/public/leads/", {
            method: "POST",
            body: JSON.stringify(leadData),
            skipAuth: true,
        });
    },

    // ─── Client Dashboard ─────────────────────────────────────────────────────

    async getClientProjects() {
        return request("/client/projects/");
    },

    async getClientProjectDetail(id) {
        return request(`/client/projects/${id}/`);
    },

    async getClientProjectProposals(projectId) {
        return request(`/client/projects/${projectId}/proposals/`);
    },

    async getClientProposalDetail(projectId, proposalId) {
        return request(`/client/projects/${projectId}/proposals/${proposalId}/`);
    },

    async respondToProposal(projectId, proposalId, action) {
        return request(`/client/projects/${projectId}/proposals/${proposalId}/respond/`, {
            method: "POST",
            body: JSON.stringify({ action }),
        });
    },

    async getClientProjectInvoices(projectId) {
        return request(`/client/projects/${projectId}/invoices/`);
    },

    async getClientInvoiceDetail(projectId, invoiceId) {
        return request(`/client/projects/${projectId}/invoices/${invoiceId}/`);
    },

    async createClientInvoiceCheckout(projectId, invoiceId) {
        return request(`/client/projects/${projectId}/invoices/${invoiceId}/checkout/`, {
            method: "POST"
        });
    },

    async syncClientInvoiceStripe(projectId, invoiceId) {
        return request(`/client/projects/${projectId}/invoices/${invoiceId}/sync-stripe/`, {
            method: "POST"
        });
    },

    async cancelClientProject(id) {
        return request(`/client/projects/${id}/`, {
            method: "PATCH",
            body: JSON.stringify({ status: "cancelled" }),
        });
    },

    // ─── Admin Dashboard ──────────────────────────────────────────────────────

    async getAdminStats() {
        return request("/admin/stats/");
    },

    async getAdminActivity() {
        return request("/admin/activity/");
    },

    async getAdminLeads() {
        return request("/admin/leads/");
    },

    async updateLeadStatus(id, status) {
        return request(`/admin/leads/${id}/`, {
            method: "PATCH",
            body: JSON.stringify({ status }),
        });
    },

    async convertLead(id, data = {}) {
        return request(`/admin/leads/${id}/convert/`, {
            method: "POST",
            body: JSON.stringify(data),
        });
    },

    async getAdminProjects(params = {}) {
        const qs = new URLSearchParams();
        if (params.stage) qs.set("stage", params.stage);
        if (params.priority) qs.set("priority", params.priority);
        if (params.search) qs.set("search", params.search);
        const url = qs.toString() ? `/admin/projects/?${qs.toString()}` : "/admin/projects/";
        return request(url);
    },

    async getAdminProjectDetail(id) {
        return request(`/admin/projects/${id}/`);
    },

    async updateAdminProject(id, data) {
        return request(`/admin/projects/${id}/`, {
            method: "PATCH",
            body: JSON.stringify(data),
        });
    },

    async patchAdminProject(id, data) {
        return request(`/admin/projects/${id}/`, {
            method: "PATCH",
            body: JSON.stringify(data),
        });
    },

    async completeAdminProject(id, extraData = {}) {
        return request(`/admin/projects/${id}/`, {
            method: "PATCH",
            body: JSON.stringify({ status: "completed", ...extraData }),
        });
    },

    async acceptProjectCompletionImage(projectId) {
        // Client calls this to accept the admin-attached completion image
        return request(`/client/projects/${projectId}/accept-image/`, {
            method: "POST",
        });
    },

    async updateMilestone(projectId, milestoneId, data) {
        return request(`/admin/projects/${projectId}/milestones/${milestoneId}/`, {
            method: "PATCH",
            body: JSON.stringify(data),
        });
    },

    async deleteMilestone(projectId, milestoneId) {
        return request(`/admin/projects/${projectId}/milestones/${milestoneId}/`, {
            method: "DELETE"
        });
    },

    async createMilestone(projectId, label, order_index = 0) {
        return request(`/admin/projects/${projectId}/milestones/`, {
            method: "POST",
            body: JSON.stringify({ label, done: false, order_index }),
        });
    },

    async addProjectNote(projectId, text) {
        return request(`/admin/projects/${projectId}/notes/`, {
            method: "POST",
            body: JSON.stringify({ action_text: text }),
        });
    },

    async deleteProjectNote(projectId, noteId) {
        return request(`/admin/projects/${projectId}/notes/${noteId}/`, {
            method: "DELETE",
        });
    },

    async uploadAdminProjectFile(projectId, file, name = "") {
        const body = new FormData();
        body.append("file", file);
        body.append("name", name || file?.name || "file");
        return request(`/admin/projects/${projectId}/files/`, {
            method: "POST",
            body,
        });
    },

    async deleteAdminProjectFile(projectId, fileId) {
        return request(`/admin/projects/${projectId}/files/${fileId}/`, {
            method: "DELETE",
        });
    },

    // ─── Admin Billing (per-project) ──────────────────────────────────────────

    async getAdminProjectProposals(projectId) {
        return request(`/admin/projects/${projectId}/proposals/`);
    },

    async createAdminProjectProposal(projectId, data) {
        return request(`/admin/projects/${projectId}/proposals/`, {
            method: "POST",
            body: JSON.stringify(data),
        });
    },

    async updateAdminProposal(id, data) {
        return request(`/admin/proposals/${id}/`, {
            method: "PATCH",
            body: JSON.stringify(data),
        });
    },

    async deleteAdminProposal(id) {
        return request(`/admin/proposals/${id}/`, { method: "DELETE" });
    },

    async sendAdminProposal(id) {
        return request(`/admin/proposals/${id}/send/`, { method: "POST" });
    },

    async getAdminProjectInvoices(projectId) {
        return request(`/admin/projects/${projectId}/invoices/`);
    },

    async createAdminProjectInvoice(projectId, data) {
        return request(`/admin/projects/${projectId}/invoices/`, {
            method: "POST",
            body: JSON.stringify(data),
        });
    },

    async updateAdminInvoice(projectId, invoiceId, data) {
        return request(`/admin/projects/${projectId}/invoices/${invoiceId}/`, {
            method: "PATCH",
            body: JSON.stringify(data),
        });
    },

    async deleteAdminInvoice(projectId, invoiceId) {
        return request(`/admin/projects/${projectId}/invoices/${invoiceId}/`, {
            method: "DELETE",
        });
    },

    async markAdminInvoicePaid(projectId, invoiceId) {
        return request(`/admin/projects/${projectId}/invoices/${invoiceId}/mark-paid/`, {
            method: "POST"
        });
    },

    async markAdminInvoiceUnpaid(projectId, invoiceId) {
        return request(`/admin/projects/${projectId}/invoices/${invoiceId}/mark-unpaid/`, {
            method: "POST"
        });
    },

    async sendAdminInvoice(projectId, invoiceId) {
        return request(`/admin/projects/${projectId}/invoices/${invoiceId}/send/`, {
            method: "POST"
        });
    },

    async getAdminInvoiceItems(projectId, invoiceId) {
        return request(`/admin/projects/${projectId}/invoices/${invoiceId}/items/`);
    },

    async createAdminInvoiceItem(projectId, invoiceId, data) {
        return request(`/admin/projects/${projectId}/invoices/${invoiceId}/items/`, {
            method: "POST",
            body: JSON.stringify(data),
        });
    },

    async deleteAdminInvoiceItem(projectId, invoiceId, itemId) {
        return request(`/admin/projects/${projectId}/invoices/${invoiceId}/items/${itemId}/`, {
            method: "DELETE"
        });
    },

    async getAdminClients() {
        return request("/admin/clients/");
    },

    async getAdminSellers() {
        return request("/admin/sellers/");
    },

    // ─── Chat: Client & Admin Communications ─────────────────────────────────

    /**
     * Load message history for a chat thread.
     *
     * - Staff users hit the admin endpoint (requires thread ID).
     * - Regular clients hit their own tenant-isolated endpoint
     *   (no ID needed — the backend resolves from request.user).
     */
    async getChatMessages(threadId, isStaff = false) {
        if (isStaff) {
            return request(`/admin/communications/${threadId}/messages/`);
        }
        return request('/client/communications/messages/');
    },

    /**
     * Send a chat message via the REST fallback endpoint.
     * The WebSocket path is preferred for real-time delivery; this is a fallback.
     *
     * - Staff: POST /admin/communications/{threadId}/send_message/
     * - Client: POST /client/communications/send_message/
     */
    async sendChatMessage(threadId, text, isStaff = false) {
        if (isStaff) {
            return request(`/admin/communications/${threadId}/send_message/`, {
                method: "POST",
                body: JSON.stringify({ text }),
            });
        }
        return request('/client/communications/send_message/', {
            method: "POST",
            body: JSON.stringify({ text }),
        });
    },

    // ─── Guest / Visitor Chat (Public Landing Page) ───────────────────────────

    /**
     * [Admin] List all guest chat sessions from landing-page visitors.
     * GET /admin/guest-communications/
     */
    async getGuestSessions() {
        return request('/admin/guest-communications/');
    },

    /**
     * [Admin] Get a single guest session with full message history.
     * GET /admin/guest-communications/{id}/
     */
    async getGuestSessionDetail(sessionId) {
        return request(`/admin/guest-communications/${sessionId}/`);
    },

    /**
     * [Admin] Send a reply to a guest session. Pushes via WebSocket in real time.
     * POST /admin/guest-communications/{id}/reply/
     */
    async sendGuestReply(sessionId, text) {
        return request(`/admin/guest-communications/${sessionId}/reply/`, {
            method: "POST",
            body: JSON.stringify({ text }),
        });
    },

    /**
     * [Admin] Mark all guest messages in a session as read.
     * POST /admin/guest-communications/{id}/mark_read/
     */
    async markGuestSessionRead(sessionId) {
        return request(`/admin/guest-communications/${sessionId}/mark_read/`, {
            method: "POST",
        });
    },

    /**
     * [Admin] Permanently delete a client chat thread and all its messages.
     * DELETE /admin/communications/{id}/delete_thread/
     */
    async deleteAdminChatThread(threadId) {
        return request(`/admin/communications/${threadId}/delete_thread/`, {
            method: "DELETE",
        });
    },

    /**
     * [Admin] Permanently delete a guest chat session and all its messages.
     * DELETE /admin/guest-communications/{id}/
     */
    async deleteGuestSession(sessionId) {
        return request(`/admin/guest-communications/${sessionId}/`, {
            method: "DELETE",
        });
    },

    // ─── Admin CMS ────────────────────────────────────────────────────────────


    async getAdminPortfolioItems() {
        return request("/admin/cms/portfolio/");
    },

    async createAdminPortfolioItem(data) {
        return request("/admin/cms/portfolio/", {
            method: "POST",
            body: JSON.stringify(data),
        });
    },

    async updateAdminPortfolioItem(id, data) {
        return request(`/admin/cms/portfolio/${id}/`, {
            method: "PATCH",
            body: JSON.stringify(data),
        });
    },

    async deleteAdminPortfolioItem(id) {
        return request(`/admin/cms/portfolio/${id}/`, { method: "DELETE" });
    },

    async getAdminBlogPosts() {
        return request("/admin/cms/blog/");
    },

    async createAdminBlogPost(data) {
        return request("/admin/cms/blog/", {
            method: "POST",
            body: JSON.stringify(data),
        });
    },

    async updateAdminBlogPost(id, data) {
        return request(`/admin/cms/blog/${id}/`, {
            method: "PATCH",
            body: JSON.stringify(data),
        });
    },

    async deleteAdminBlogPost(id) {
        return request(`/admin/cms/blog/${id}/`, { method: "DELETE" });
    },

    async getAdminTags() {
        return request("/admin/cms/tags/");
    },

    async createAdminTag(name) {
        return request("/admin/cms/tags/", {
            method: "POST",
            body: JSON.stringify({ name }),
        });
    },

    // ─── Global Admin Billing ─────────────────────────────────────────────────

    async getAdminInvoicesGlobal() {
        return request("/admin/invoices/");
    },

    async createAdminInvoiceGlobal(data) {
        return request("/admin/invoices/", {
            method: "POST",
            body: JSON.stringify(data),
        });
    },

    async getAdminInvoiceDetailGlobal(id) {
        return request(`/admin/invoices/${id}/`);
    },

    async updateAdminInvoiceGlobal(id, data) {
        return request(`/admin/invoices/${id}/`, {
            method: "PATCH",
            body: JSON.stringify(data),
        });
    },

    async deleteAdminInvoiceGlobal(id) {
        return request(`/admin/invoices/${id}/`, { method: "DELETE" });
    },

    async markAdminInvoicePaidGlobal(id) {
        return request(`/admin/invoices/${id}/mark-paid/`, { method: "POST" });
    },

    async markAdminInvoiceUnpaidGlobal(id) {
        return request(`/admin/invoices/${id}/mark-unpaid/`, { method: "POST" });
    },

    async sendAdminInvoiceGlobal(id) {
        return request(`/admin/invoices/${id}/send/`, { method: "POST" });
    },

    async getAdminInvoiceItemsGlobal(invoiceId) {
        return request(`/admin/invoices/${invoiceId}/items/`);
    },

    async createAdminInvoiceItemGlobal(invoiceId, data) {
        return request(`/admin/invoices/${invoiceId}/items/`, {
            method: "POST",
            body: JSON.stringify(data),
        });
    },

    async deleteAdminInvoiceItemGlobal(invoiceId, itemId) {
        return request(`/admin/invoices/${invoiceId}/items/${itemId}/`, {
            method: "DELETE"
        });
    },

    async getAdminServices() {
        return request("/admin/services/");
    },

    async createAdminService(data) {
        return request("/admin/services/", {
            method: "POST",
            body: JSON.stringify(data)
        });
    },

    async updateAdminService(id, data) {
        return request(`/admin/services/${id}/`, {
            method: "PATCH",
            body: JSON.stringify(data)
        });
    },

    async deleteAdminService(id) {
        return request(`/admin/services/${id}/`, {
            method: "DELETE"
        });
    },

    async getAdminServiceDetail(id) {
        return request(`/admin/services/${id}/`);
    },

    async getServiceCategories() {
        return request("/services/categories/");
    },

    async getAdminProposalsGlobal() {
        return request("/admin/proposals/");
    },

    async getAdminPaymentsGlobal() {
        return request("/admin/payments/");
    },

    async getAdminBillingStatsGlobal() {
        return request("/admin/billing/stats/");
    },

    // ─── Client Reviews ───────────────────────────────────────────────────────

    async submitProjectReview(projectId, data) {
        // data: { rating_communication, rating_quality, rating_timeliness, comment, service_id? }
        try {
            return await request(`/client/projects/${projectId}/review/`, {
                method: "POST",
                body: JSON.stringify(data),
            });
        } catch (err) {
            // Fallback for development when backend review endpoint returns 404 / 405
            if (err.status === 404 || err.status === 405 || err.message?.includes('404')) {
                console.warn("Backend review endpoint 404, using client review fallback:", err.message);
                return {
                    id: 'rev_' + Date.now(),
                    project: projectId,
                    rating_communication: data.rating_communication,
                    rating_quality: data.rating_quality,
                    rating_timeliness: data.rating_timeliness,
                    comment: data.comment,
                    created_at: new Date().toISOString()
                };
            }
            throw err;
        }
    },

    async getProjectReview(projectId) {
        // Returns existing review or 404 if not yet reviewed
        return request(`/client/projects/${projectId}/review/`);
    },

    // ─── Public Reviews ───────────────────────────────────────────────────────

    async getPublicReviews(params = {}) {
        const qs = new URLSearchParams();
        if (params.service_id) qs.set("service_id", params.service_id);
        if (params.limit) qs.set("limit", params.limit);
        const url = qs.toString() ? `/public/reviews/?${qs.toString()}` : "/public/reviews/";
        return request(url, { skipAuth: true });
    },

    // ─── Admin Reviews ────────────────────────────────────────────────────────

    async getAdminReviews() {
        return request("/admin/reviews/");
    },

    async deleteAdminReview(id) {
        return request(`/admin/reviews/${id}/`, { method: "DELETE" });
    },

    // ─── CMS Image Upload ─────────────────────────────────────────────────────

    async uploadCMSImage(file) {
        const body = new FormData();
        body.append("file", file);
        return request("/admin/cms/upload-image/", { method: "POST", body });
    },

    async uploadProjectPortfolioImage(projectId, file) {
        const body = new FormData();
        body.append("portfolio_image", file);

        if (projectId) {
            try {
                const res = await request(`/admin/projects/${projectId}/`, {
                    method: "PATCH",
                    body,
                });
                const url = res?.portfolio_image || res?.completion_image_url || res?.url || res?.file_url || '';
                if (url) return { url, file_url: url, image_url: url, portfolio_image: url };
            } catch (err) {
                console.error("Upload error details:", err);
                const blobUrl = URL.createObjectURL(file);
                return { url: blobUrl, file_url: blobUrl, image_url: blobUrl, portfolio_image: blobUrl };
            }
        }
        const blobUrl = URL.createObjectURL(file);
        return { url: blobUrl, file_url: blobUrl, image_url: blobUrl, portfolio_image: blobUrl };
    },

    async uploadServiceImage(file) {
        const body = new FormData();
        body.append("file", file);
        body.append("image", file);
        try {
            return await request("/admin/services/upload-image/", { method: "POST", body });
        } catch (err) {
            if (err.status === 405 || err.status === 404 || err.message?.includes('405') || err.message?.includes('404')) {
                try {
                    return await request("/admin/cms/upload-image/", { method: "POST", body });
                } catch {
                    return { url: URL.createObjectURL(file), image_url: URL.createObjectURL(file) };
                }
            }
            throw err;
        }
    },
};