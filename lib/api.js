...
export const api = {
    ...
    async uploadProjectPortfolioImage(projectId, file) {
        const body = new FormData();
        body.append("completion_image", file);
        ...
        if (projectId) {
            try {
                ...
                const url = res?.completion_image_url || res?.portfolio_image || res?.completion_image || res?.image || '';
                if (url) return { url, file_url: url, image_url: url };
            } catch (err) {
                const blobUrl = URL.createObjectURL(file);
                return { url: blobUrl, file_url: blobUrl, image_url: blobUrl };
            }
        }
        const blobUrl = URL.createObjectURL(file);
        return { url: blobUrl, file_url: blobUrl, image_url: blobUrl };
