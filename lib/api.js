...
async function request(url, options = {}) {
  ...
  // Connect directly to the Django backend to bypass local dev proxy / DNS loopback issues on Windows
  const finalUrl = url.startsWith('/') ? `http://127.0.0.1:8000${url}` : url;
