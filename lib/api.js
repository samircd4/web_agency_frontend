...
async function request(url, options = {}) {
  ...
  // Safely construct the final URL without breaking string modifications
  let finalUrl = url;
  if (url.startsWith("/")) {
    // Strips trailing slash from base if present, strips leading slash from path
    finalUrl = `${apiBaseUrl.replace(/\/$/, "")}${url}`;
  }
...
export const api = {
  ...
  async updateMe(data) {
    ...
    return request("/api/auth/me/", { method: "PATCH", body: formData });
  },
  ...
  async otpGenerate() {
    return request("/api/auth/otp/generate/", { method: "POST" });
  },
  ...
  async otpDisable(data) {
    return request("/api/auth/otp/disable/", {
      method: "POST",
      body: JSON.stringify(data),
    });
  },
  ...
  async revokeSession(id) {
    const refreshToken = localStorage.getItem("refresh_token");
    return request(`/api/auth/sessions/${id}/revoke/`, {
      method: "POST",
      body: JSON.stringify({ refresh_token: refreshToken }),
    });
  },
  ...
  async revokeAllSessions() {
    const refreshToken = localStorage.getItem("refresh_token");
    return request("/api/auth/sessions/revoke-all/", {
      method: "POST",
      body: JSON.stringify({ refresh_token: refreshToken }),
    });
  },
  ...
  async respondToProposal(projectId, proposalId, action) {
    return request(`/api/client/projects/${projectId}/proposals/${proposalId}/respond/`, {
      method: "POST",
  ...
  async createClientInvoiceCheckout(projectId, invoiceId) {
    return request(`/api/client/projects/${projectId}/invoices/${invoiceId}/checkout/`, { 
      method: "POST" 
    });
  },

  async syncClientInvoiceStripe(projectId, invoiceId) {
    return request(`/api/client/projects/${projectId}/invoices/${invoiceId}/sync-stripe/`, { 
      method: "POST" 
    });
  },
  ...
  async markAdminInvoiceUnpaidGlobal(id) {
    return request(`/api/admin/invoices/${id}/mark-unpaid/`, { method: "POST" });
  },
