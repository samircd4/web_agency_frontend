...
export default function OrderModal({ isOpen, onClose, service, tier }) {
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            const isLikelyJwt = Boolean(rawToken && rawToken.split('.').length === 3);

            const sendCheckoutRequest = async (withAuth) => {
                const response = await fetch(`${API_BASE}/payments/create-session/`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        ...(withAuth && isLikelyJwt ? { Authorization: `Bearer ${rawToken}` } : {})
                    },
                    body: JSON.stringify({
                        name: formData.name,
                ...
                if (!response.ok) {
                    ...
                    const invalidJwt = detail.includes('token not valid') || detail.includes('given token not valid');

                    if (withAuth && invalidJwt) {
                        localStorage.removeItem('access_token');
                        localStorage.removeItem('refresh_token');
                        document.cookie = 'admin_session=; path=/; expires=Thu, 01 Jan 1970 00:00:01 GMT;';
                        return sendCheckoutRequest(false);
                    }
            ...
            const data = await sendCheckoutRequest(true);
