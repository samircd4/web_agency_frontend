...
export default function DashboardView() {
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (checkout) {
                // Clean URL parameters by replacing state
                if (typeof window !== 'undefined') {
                                                                ...
                                                                <tr key={`${inv._projectId}-${inv.id}`} className={`${idx % 2 === 0 ? 'bg-white/[0.01]' : ''} hover:bg-white/[0.04] transition-colors`}>
                                                                    ...
                                                                    <td className="px-6 py-4 text-right">
                                                                        <div className="flex items-center justify-end gap-2">
                                                                            ...
                                                                            <button
                                                                                type="button"
                                                                                onClick={() => { const t = localStorage.getItem('access_token') || ''; const q = t ? `?token=${encodeURIComponent(t)}` : ''; window.open(`${API_BASE_URL}/client/projects/${inv._projectId}/invoices/${inv.id}/print/${q}`, '_blank', 'noopener,noreferrer'); }}
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="p-4 bg-white/[0.02] border-t border-white/5 flex items-center justify-end gap-2">
                                <button
                                    type="button"
                                    onClick={() => { const t = localStorage.getItem('access_token') || ''; const q = t ? `?token=${encodeURIComponent(t)}` : ''; window.open(`${API_BASE_URL}/client/projects/${selectedInvoice._projectId}/invoices/${selectedInvoice.id}/print/${q}`, '_blank', 'noopener,noreferrer'); }}
