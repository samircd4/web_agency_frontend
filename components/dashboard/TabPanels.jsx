  ...
  return (
    <motion.div key="billing" initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }}>
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        {billingDocsLoading && <span className="ml-2 text-[10px] font-black uppercase tracking-widest text-slate-600">Loading…</span>}
              ...
              <table className="min-w-[900px] w-full text-left">
                ...
                <tbody className="divide-y divide-white/10">
                  {clientProposals.length === 0 ? (
                    <tr>
                      <td colSpan="6" className="px-6 py-10 text-center text-slate-400 text-sm">No proposals yet.</td>
                    </tr>
                  ) : (
...
export default function DashboardTabPanels(props) {
  return (
    <AnimatePresence mode="wait">
      {props.activeTab === 'missions' && (
        <MissionsTab {...props} />
      )}
      {props.activeTab === 'vault' && <VaultTab vaultFiles={props.vaultFiles} />}
      {props.activeTab === 'comms' && <CommsTab missions={props.missions} />}
