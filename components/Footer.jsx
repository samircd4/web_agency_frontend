// Footer.jsx (NO 'use client' - now a Server Component!)
            ...
            <div className="container mx-auto px-4">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8 lg:gap-8 mb-12 md:mb-20">
                    <div className="lg:col-span-2">
                        <Link href="/" prefetch={false} className="flex items-center gap-3 mb-8 group">
                        ...
                        <div className="flex gap-4">
                            {[
                                { icon: <Code2 size={20} />, label: "GitHub", href: '#', color: "hover:border-brand-teal/30 hover:text-brand-teal" },
                                { icon: <Share2 size={20} />, label: "LinkedIn", href: '#', color: "hover:border-brand-red/30 hover:text-brand-red" },
                                { icon: <Globe size={20} />, label: "Website", href: '/', color: "hover:border-brand-blue/30 hover:text-brand-blue" },
                                { icon: <Mail size={20} />, label: "Email", href: 'mailto:contact@drpythonsolutions.com', color: "hover:border-brand-indigo/30 hover:text-brand-indigo" }
                            ].map((social, i) => (
                                <a 
                                    key={i}
                                    title={social.label}
                                    href={social.href}
                                    className={`w-12 h-12 glass rounded-xl flex items-center justify-center text-slate-400 ${social.color} hover:-translate-y-1 hover:scale-105 transition-all duration-300`}
                                >
                                    {social.icon}
                                </a>
                            ))}
                    ...
                    <div>
                        ...
                        <ul className="space-y-4">
                            {[
                                ...
                                <li key={item.name}>
                                    <Link href={item.href} prefetch={false} className="text-slate-400 hover:text-brand-teal transition-colors font-medium">{item.name}</Link>
                                </li>
                            ))}
                        </ul>
                    </div>
                    ...
                    <div>
                        ...
                        <ul className="space-y-4">
                            {[
                                ...
                                <li key={item.name}>
                                    <Link href={item.href} prefetch={false} className="text-slate-400 hover:text-brand-red transition-colors font-medium">{item.name}</Link>
                                </li>
                            ))}
                        </ul>
                    </div>
                ...
                <div className="pt-8 md:pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6 text-slate-500 text-sm font-bold">
                    ...
                    <div className="flex gap-8">
                        <Link href="/privacy" prefetch={false} className="hover:text-white transition-colors">Privacy Protocol</Link>
