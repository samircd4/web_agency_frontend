...
export default function PortfolioDetailPage() {
  ...
  const completionDate = "Jan 2026";
  const duration = "8 Weeks";
  const budget = "$10k - $20k";

  const infoItems = [
    { label: 'Completion', value: completionDate, icon: Calendar },
    { label: 'Duration', value: duration, icon: Clock },
    { label: 'Budget Range', value: budget, icon: DollarSign }
  ];

  const challenge = project.description;
  const solution = `We implemented an industrial scale production deployment utilizing automated state reconciliation protocols to achieve maximum integrity under varying workloads.`;
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center mb-16">
          <div>
            <ScrollReveal>
              ...
              <h1 className="text-5xl md:text-7xl font-black text-white mb-8 tracking-tighter leading-[0.9]">
                ...
                <span className={isTeal ? 'text-gradient-teal' : 'text-gradient-red'}>
              ...
              <div className="flex flex-wrap gap-4">
                <a 
                  href="https://drpython.solutions" 
                  target="_blank" 
                  rel="noopener noreferrer"
                  className={`inline-flex items-center gap-3 px-8 py-4 ${isTeal ? 'bg-brand-teal shadow-glow-teal' : 'bg-brand-red shadow-glow-red'} text-white rounded-2xl font-black uppercase tracking-widest text-xs transition-all hover:-translate-y-1`}
                >
                  Live Demo <Globe size={16} />
                </a>
                
                <a 
                  href="https://github.com/drpython" 
                  target="_blank" 
                  rel="noopener noreferrer"
          ...
          <div className="relative aspect-[4/3] rounded-3xl overflow-hidden border border-white/10 shadow-2xl">
            ...
            <div className="absolute bottom-8 left-8 right-8 p-8 glass rounded-2xl border border-white/10 flex items-center justify-between">
              <div>
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">{isTeal ? "Throughput" : "Latency"}</p>
                <p className={`text-3xl font-black ${isTeal ? 'text-brand-teal' : 'text-brand-red'}`}>{isTeal ? "5.2M / day" : "< 85ms"}</p>
              </div>
              <div className={`p-4 rounded-2xl ${isTeal ? 'bg-brand-teal/10 text-brand-teal' : 'bg-brand-red/10 text-brand-red'}`}>
          ...
          <div className="mb-24">
            ...
            <div className="p-12 rounded-3xl glass border border-white/5 bg-gradient-to-br from-white/[0.02] to-transparent">
              <p className="text-slate-300 text-lg leading-relaxed font-medium">
                {project.content_markdown || project.description}
              </p>
            </div>
          </div>
        </ScrollReveal>
        ...
        <div className="text-center">
          <ScrollReveal>
            ...
            <Link href="/start-project" className="inline-flex items-center gap-3 px-10 py-5 glass text-white rounded-2xl font-black uppercase tracking-widest text-sm hover:bg-white/10 transition-all">
